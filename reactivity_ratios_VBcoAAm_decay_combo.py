...
x_data = np.concatenate((x_data_VBf05_1, x_data_VBf05_2, x_data_VBf03_1, x_data_VBf01_1, x_data_VBf07_1))
f1_data = np.concatenate((f1_data_VBf05_1, f1_data_VBf05_2, f1_data_VBf03_1, f1_data_VBf01_1, f1_data_VBf07_1))

lf1 = len(f1_data_VBf05_1)
lf2 = len(f1_data_VBf05_2)
lf3 = len(f1_data_VBf03_1)
lf4 = len(f1_data_VBf01_1)
lf5 = len(f1_data_VBf07_1)

f1_1 = f1_data[:lf1]
f1_2 = f1_data[lf1:(lf2+lf1)]
f1_3 = f1_data[(lf2+lf1):(lf3+lf2+lf1)]
f1_4 = f1_data[(lf3+lf2+lf1):(lf4+lf3+lf2+lf1)]
f1_5 = f1_data[(lf4+lf3+lf2+lf1):]
...
def combofitfunc(x,r1,r2):
    ...
    l1 = len(x_data_VBf05_1)
    l2 = len(x_data_VBf05_2)
    l3 = len(x_data_VBf03_1)
    l4 = len(x_data_VBf01_1)
    l5 = len(x_data_VBf07_1)
    
    x1 = x[:l1]
    x2 = x[l1:(l2+l1)]
    x3 = x[(l2+l1):(l3+l2+l1)]
    x4 = x[(l3+l2+l1):(l4+l3+l2+l1)]
    x5 = x[(l4+l3+l2+l1):]
...
#    x3int = interpolate.interp1d(x3,f1_3)
    ...
    f5_soln = odeint(df1dx,f1_5[0],x5)
    
    return np.concatenate((f1_soln[:,0],f2_soln[:,0],f3_soln[:,0],f4_soln[:,0],f5_soln[:,0]))
...
odr = ODR(rxn_data, model, [2.0,0.3])
...
odrfit4 = fittedfunc(xfit,f1_4[0],output.beta[0],output.beta[1])
odrfit5 = fittedfunc(xfit,f1_5[0],output.beta[0],output.beta[1])

plt.plot(x_data_VBf05_1,1-f1_data_VBf05_1,'o')
plt.plot(x_data_VBf05_2,1-f1_data_VBf05_2,'o')
plt.plot(x_data_VBf03_1,1-f1_data_VBf03_1,'o')
plt.plot(x_data_VBf01_1,1-f1_data_VBf01_1,'o')
plt.plot(x_data_VBf07_1,1-f1_data_VBf07_1,'o')
#plt.plot(xfit,fit1,'-')
#plt.plot(xfit,fit2,'-')
#plt.plot(xfit,fit3,'-')
#plt.plot(xfit,fit4,'-')
#plt.plot(xfit,fit5,'-')
plt.plot(xfit,1-odrfit1,'-')
plt.plot(xfit,1-odrfit2,'-')
plt.plot(xfit,1-odrfit3,'-')
plt.plot(xfit,1-odrfit4,'-')
plt.plot(xfit,1-odrfit5,'-')

plt.xlabel(r'$\mathrm{x} $', fontsize='large')
plt.ylabel(r'$\mathrm{f_{Am}} $', fontsize='large')

plt.ylim(0,1)
plt.xlim(0,1)
plt.savefig('react_ratio_VBcoAm2.png', bbox_inches='tight')
